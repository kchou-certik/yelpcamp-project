        ...
        if(err){
            ...
            Comment.create(req.body.comment, function(err, commentCreated){
                if(err){
                    ...
                    commentCreated.save();
                    campground.comments.push(commentCreated._id);
                    campground.save();
...
router.get("/:comment_id/edit", middleware.checkCommentOwnership, function(req, res){
    Comment.findById(req.params.comment_id, function(err, commentFound){
        if(err){
            ...
            res.render("comments/edit", {campgroundId: req.params.id, comment: commentFound});
        }
    });
});
...
router.put("/:comment_id", middleware.checkCommentOwnership, function(req, res){
    Comment.findByIdAndUpdate(req.params.comment_id, req.body.comment, function(err, commentChanged){
        if(err){
            res.redirect("back");
        } else {
